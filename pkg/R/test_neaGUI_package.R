.test <- function(dir, pattern = ".*\\.R$")
{
    ...
    if (missing(dir)) {
        dir <- system.file("unitTests", package="neaGUI")
        if (!length(dir)) {
            dir <- system.file("UnitTests", package="neaGUI")
            if (!length(dir))
    ...
    suite <- defineTestSuite(name="neaGUI RUnit Tests", dirs=dir,
    ...
    if (length(details <- .failure_details(result)) >0) {
        ...
        cat("\n\n")
        stop("unit tests failed for package neaGUI ")
    }
    result
}

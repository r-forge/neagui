...
min = 0, max = nperm, width = 300)
   ...
   if (fgslib!="KEGG.db" && fgslib!="GO.db") {
   if (is.character(fgs) && fgs != "MF" && fgs != "BP" && fgs!="CC" && fgs != "KEGG") {
        fgs.type = 5
        fgs.list <- list()
        fgs.list[[1]] <- fgs
        fgs <- fgs.list
...
require(org.Hs.eg.db)
 ...
 fgs[[i]] <- unique(names(xx[[i]])) 
    ...
    if (fgs.type < 4) {
        fgs.probe <- as.list(get(paste(fgslib, "GO2PROBE", sep = "")))
        fgsname <- names(fgs.probe)
        ont <- Ontology(fgsname)
    }
    if (fgs.type == 1) {
        fgs.probe <- fgs.probe[which(ont == "MF")]
    }
    if (fgs.type == 2) {
        fgs.probe <- fgs.probe[which(ont == "BP")]
    }
    if (fgs.type == 3) {
        fgs.probe <- fgs.probe[which(ont == "CC")]
    }
    if (fgs.type < 5) {

...
res <-neaMod(ags=AGS, fgs = FGS, fgslib  = dbInput, network=NETWORK, nperm = Perm, seed = Seed, pnet=pnet, stat=rbValstat )
...
options(warn=-1)
      PathID1 <- sapply(PathID , function (x) getcode(x)  )
         options(warn=1)
      combres <-   cbind(PATH_ID = PathID1, combres )
pathId <- as.list(KEGGPATHID2NAME)

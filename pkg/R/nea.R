...
nea <-function (ags, fgs, fgslib = NULL, network, pnet = NULL, nperm = 50,stat = "F", seed = NULL) 
{
	...
	if (stat == "F") {
		...
		meanlink <- apply(stac.nlink, 1, mean)
		se <- apply(stac.nlink, 1, sd)
		se <- ifelse(se == 0, 1, se)
		stac.zscore <- (stac.nlink - meanlink )/se
		...
		##
		FDR <- pval2FDR(pvalue)
		MainResult <- data.frame(Number_links= nlink,  Expected_links = meanlink , Number_of_Genes=ngenefgs,
		Number_of_AGS_genes=numgene, Z_score=zscore, P_value=pvalue,  FDR= FDR)
		
		return(list( MainResult= MainResult   ,geneinfgs=geneinfgs, fgs.list =fgs.list, pnetout = pnetout))
	}
	if (stat == "M") {
